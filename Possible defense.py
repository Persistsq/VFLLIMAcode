...
class VflFramework(nn.Module):
    ...
    def __init__(self):
        ...
        #collect grad for bottom output
        self.collect_imd_grad = False
        self.imd_grad = None
        self.outputs_a_all = None
        ...
        self.flsg = args.flsg
        self.VFLDefender = args.VFLDefender
    ...
    def simulate_train_round_per_batch(self, data, target, indices):
        timer_mal = 0
        timer_benign = 0
        ...
        if self.VFLDefender:
            replace_grad_list = []
            tau = 8
            for passive_grad in model_all_layers_grads_list:
                # grad_list = passive_grad.detach().clone().cpu().view(-1).tolist()
                grad_list = passive_grad.detach().clone().cpu().view(-1)
                clipped_grad = torch.clamp(grad_list, min=torch.min(grad_list).item(), max=torch.max(grad_list).item())
                norm = torch.norm(clipped_grad)
                normalized_grad = clipped_grad / (norm + 1e-8)


                random_grad = torch.zeros_like(normalized_grad)
                positive_mask = normalized_grad > 0
                negative_mask = normalized_grad < 0


                random_grad[positive_mask] = torch.rand(1) * torch.max(normalized_grad) / tau
                random_grad[negative_mask] = torch.rand(1) * torch.min(normalized_grad) / tau


                replace_list = random_grad.tolist()


                replace_grad_list.append(torch.Tensor(replace_list).view_as(passive_grad).cuda())

            model_all_layers_grads_list = replace_grad_list
        ...
        #collect imd grad for malicious
        if self.collect_imd_grad:
            batch_grad_list = grad_output_bottom_model_a.detach().clone().cpu().view(-1).tolist()
            l_bgl = len(batch_grad_list)
            for i in range(len(indices)):
                self.imd_grad[indices[i]].extend(batch_grad_list[int(l_bgl/len(target)*i):int(l_bgl/len(target)*(i+1))])

            batch_output_a_list = input_tensor_top_model_a.detach().clone().cpu().view(-1).tolist()
            l_bgl = len(batch_grad_list)
            for i in range(len(indices)):
                self.outputs_a_all[indices[i]].extend(batch_output_a_list[int(l_bgl/len(target)*i):int(l_bgl/len(target)*(i+1))])
...
def main():
    ...
    train_dataset, val_loader = set_loaders()

    dir_save_model = args.save_dir + f"_half{args.half}/saved_models/{args.dataset}_saved_models"
    ...
    recordcount = 0.1
    model.collect_imd_grad = True
    model.imd_grad = []
    for i in range(len(train_dataset)):
        model.imd_grad.append([])
    model.outputs_a_all = []
    for i in range(len(train_dataset)):
        model.outputs_a_all.append([])
    for epoch in range(args.epochs):
        print(f"epoch:{epoch}")
        print('model.optimizer_top_model current lr {:.5e}'.format(
            model.optimizer_top_model.param_groups[0]['lr']))
        ...
        train_loader, indices = random_train_loader(train_dataset)
        start_idx = 0
        end_idx = 0
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            if args.dataset == 'Yahoo':
                ...
                target = target.long().cuda()
            end_idx += len(target)
            loss_framework = model.simulate_train_round_per_batch(data, target, indices[start_idx:end_idx])
            start_idx = end_idx
            if batch_idx % 25 == 0:
                if args.dataset == 'Criteo':
        ...
        labels_list = []
        for i in range(len(train_dataset)):
            labels_list.append(train_dataset[i][1])
        if (epoch+1) % (10) == 0:

            df_gradA = pd.DataFrame(data=model.imd_grad, index=labels_list)
            dir_save_tsne_pic = args.save_dir + f"_half{args.half}/csv_files/{args.dataset}_csv_files"
            if not os.path.exists(dir_save_tsne_pic):
                os.makedirs(dir_save_tsne_pic)
            df_gradA.to_csv(
                dir_save_tsne_pic + f"/{args.dataset}_imdgrad_all_{args.half}_{epoch}.csv")
            model.imd_grad = []
            for i in range(len(train_dataset)):
                model.imd_grad.append([])
            
            df_output_a = pd.DataFrame(data=model.outputs_a_all, index=labels_list)
            df_output_a.to_csv(
                dir_save_tsne_pic + f"/{args.dataset}_outputA_all_{args.half}_{epoch}.csv")
            model.outputs_a_all = []
            for i in range(len(train_dataset)):
                model.outputs_a_all.append([])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save-dir', dest='save_dir',
                        ...
                        default='./saved_experiment_results_flsg', type=str)
    ...
    parser.add_argument('--flsg', help='turn on flsg',
                        type=ast.literal_eval, default=False)
    parser.add_argument("--VFLDefender", help="turn on VFLDefender",
                        type=ast.literal_eval, default=False)
